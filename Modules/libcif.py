...
# Modified by Denis Spasyuk: dsn.selfip.com (Canada); Email: denis.spasyuk@gmail.com
...
import os.path, sys, re,time, os, string, subprocess
...
NeededForRST={"_chemical_name_common":"Name of the molecule",
			...
			"_diffrn_reflns_number":"Total number of reflections" ,
			"_reflns_number_total": "Total number of UNIC reflections",
...
#			"_computing_data_collection":"Data collection",
			...
			"_refine_ls_number_reflns":"Number of reflections",			
...
'_refln_F-squared_meas','_refln_F_calc','_refln_F_meas','_refln_F_sigma', '_platon_squeeze_void_nr', '_platon_squeeze_void_average_x',      
'_platon_squeeze_void_average_y', '_platon_squeeze_void_average_z', '_platon_squeeze_void_volume', '_platon_squeeze_void_count_electrons',
'_platon_squeeze_void_content', '_platon_squeeze_details', 
...
Version=[" Generated by libcif.py : June 2005"," Written by Jerome Kieffer : Jerome.Kieffer@terre-adelie.org; Modified by Denis Spasyuk: dsn.selfip.com (Canada)"," X-Ray Crystallography "," (France)"]
...
def platon(path,filename):
	...
	i,o=subprocess.Popen(path+"/bin/platon -p %s"%filename)
	i.write("TABL CIF\n")
	i.flush()
...
#########################################################################
###### Everything you need to write a report automaticaly #######################
#########################################################################
...
def XraySummary(html,cif):
	...
	html.element("td","%s, %s g/cm³"%(cif["_cell_formula_units_Z"],cif["_exptl_crystal_density_diffrn"]))
	html.end("tr")
	html.start("tr")
	...
	html.end("tr")
	html.start("tr")
	html.element("td","Reflection collected / unique")
...
def Appendix(filename,cif,Lang="En"):
	"""This function writes out an X-Ray structure report as an HTML file in the given language
	with the data taken from the CIF dictionnary
...
def structureImage(path,basename,structure="structure"):
	...
	if structure.lower()=="powder":
		...
		while not os.path.isfile(basename+".ps"):
			i=os.popen(self.path+"/bin/platon -a %s.cif"%(basename))
		...
		while not os.path.isfile(basename+".ps"):
			i=subprocess.Popen(path+"/bin/platon -p %s.cif"%(basename), shell=True, stdin=subprocess.PIPE,
			stdout=subprocess.PIPE)
			stdin_value, stdout_value = i.communicate()
			if structure.lower()=="adp":
				i.stdin.write("plot adp\n")
			else:
				i.stdin.write("pluton\n")
			i.flush()
			i.stdin.write("menu on\n")
			i.flush()
			for l in i.stdout.readlines(): print (l.strip())
			i.close()
